...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/ayar.yaml', help='config file')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = SAGNN(layers=args.layers, classes=2, zoom_factor=8, \
                   criterion=nn.CrossEntropyLoss(ignore_index=255), BatchNorm=BatchNorm, \
                   pretrained=True, shot=args.shot, ppm_scales=args.ppm_scales, device_number=args.train_gpu[0],no_pass=1)
    ...
    model.to('cuda:{}'.format(args.train_gpu[0]))
    optimizer = torch.optim.SGD(
        [
            ...
            {'params': model.layer6_0.parameters()},
            {'params': model.layer6_1.parameters()},
            {'params': model.layer6_2.parameters()},
            {'params': model.layer6_3.parameters()},
            {'params': model.layer6_4.parameters()},
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_log = epoch + 1
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, optimizer, epoch, args.train_gpu[0])
        if main_process():
        ...
        if args.evaluate and (epoch % 2 == 0 or (args.epochs <= 50 and epoch % 1 == 0)):
            loss_val, mIoU_val, mAcc_val, allAcc_val, class_miou = validate(val_loader, model, criterion,args.train_gpu[0])
            if main_process():
...
def train(train_loader, model, optimizer, epoch, device):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        s_input = s_input.cuda(device=device,non_blocking=True)
        s_mask = s_mask.cuda(device=device,non_blocking=True)
        input = input.cuda(device=device,non_blocking=True)
...
def validate(val_loader, model, criterion,device):
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            s_input = s_input.cuda(device=device, non_blocking=True)
            target = target.cuda(device=device,non_blocking=True)
            ori_label = ori_label.cuda(device=device,non_blocking=True)
            s_mask = s_mask.cuda(device=device,non_blocking=True)
            ...
            if args.ori_resize:
                ...
                backmask = torch.ones(ori_label.size(0), longerside, longerside).cuda(device) * 255
